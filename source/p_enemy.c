...
static boolean P_LookForPlayers(mobj_t *actor, boolean allaround)
{
    ...
    {  // killough 9/9/98: friendly monsters go about players differently
        ...
        for (anyone=0; anyone<=1; anyone++)
                if (_g->playeringame && _g->players[0].playerstate==PST_LIVE &&
                        (anyone || P_IsVisible(actor, _g->players[0].mo, allaround)))
                {
                    P_SetTarget(&actor->target, _g->players[0].mo);
...
void A_BossDeath(mobj_t *mo)
{
  ...
  if (!(_g->playeringame && _g->players[0].health > 0))
